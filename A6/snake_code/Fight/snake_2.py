...
class Snake:
    ...
    def __init__(self, color, pos, file_name=None):
        ...
        self.epsilon = 0.005
        self.epsilon_decay = 0.99901  # Epsilon decay for further runs
    ...
    def update_q_table(self, state, action, next_state, reward):
        self.q_table[state[0], state[1], action] = (1 - self.lr) * self.q_table[
            state[0], state[1], action] + self.lr * (reward + self.discount_factor * np.max(
    ...
    def observe_obstacles(self, other_snake):
        ...
        # Check left
        if head_x == 1 or (head_x - 1, head_y) in [cube.pos for cube in self.body] or (head_x - 1, head_y) in [cube.pos
                                                                                                               for cube
                                                                                                               in
                                                                                                               other_snake.body]:
        ...
        # Check right
        if head_x == ROWS - 2 or (head_x + 1, head_y) in [cube.pos for cube in self.body] or (head_x + 1, head_y) in [
            cube.pos for cube in other_snake.body]:
            obstacles += 4  # Set the second bit (2^2)

        # Check up
        if head_y == 1 or (head_x, head_y - 1) in [cube.pos for cube in self.body] or (head_x, head_y - 1) in [cube.pos
                                                                                                               for cube
                                                                                                               in
                                                                                                               other_snake.body]:
        ...
        # Check down
        if head_y == ROWS - 2 or (head_x, head_y + 1) in [cube.pos for cube in self.body] or (head_x, head_y + 1) in [
            cube.pos for cube in other_snake.body]:
    ...
    def dist_to_snack(self, snack, pos):
        return abs(pos[0] - snack.pos[0]) + abs(pos[1] - snack.pos[1])
    ...
    def calc_reward(self, snack, other_snake):
        ...
        if self.head.pos == snack.pos:
            ...
            reward = 50  # Reward the snake for eating
        ...
        if self.head.pos in list(map(lambda z: z.pos, other_snake.body)):
            ...
            # if reward == 0:
            #     if self.dist_to_snack(snack, self.prev_head.pos) > self.dist_to_snack(snack, self.head.pos):
            #         reward = 2
            #     else:
            #         reward = -3
            # reset(self, other_snake)
